...
/**
 * Strip the zone before writing a time.
 *
 * The column is `timestamptz`, so Postgres converts anything carrying an offset
 * to UTC — "11:00-04:00" comes back as "15:00+00:00". This app reads times as
 * wall clock (a 11am lunch in Toronto is 11am), so that round trip moves a
 * booking by its offset the first time it's saved. Store the clock face; the
 * zone lives in the place, not the column.
 */
const wallClock = (iso?: string | null) =>
  iso ? iso.replace(/(\.\d+)?(Z|[+-]\d{2}:\d{2})$/, "") : iso;
...
export function bookingToRow(b: Partial<Booking>): Partial<BookingRow> {
  ...
  if (b.eventAt !== undefined) row.event_at = wallClock(b.eventAt)!;
  if (b.checkOut !== undefined) row.check_out = wallClock(b.checkOut) ?? null;
  if (b.arriveAt !== undefined) row.arrive_at = wallClock(b.arriveAt) ?? null;
  ...
  if (b.cancelBy !== undefined) row.cancel_by = wallClock(b.cancelBy) ?? null;
