...
export function RefundBadge({ booking }: { booking: Booking }) {
  ...
  if (booking.status === "tobook") {
    return (
      <span className="inline-flex items-center gap-1 rounded-full border border-dashed border-accent px-2 py-0.5 text-[12px] font-medium text-accent">
        To book
      </span>
    );
  }
  if (booking.status === "plan") {
    return (
      <span className="inline-flex items-center gap-1 rounded-full border border-dashed border-line-strong px-2 py-0.5 text-[12px] font-medium text-ink-soft">
        Planned
      </span>
    );
  }
  if (!booking.refundable) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-locked-soft px-2 py-0.5 text-[12px] font-medium text-locked">
        Non-refundable
      </span>
    );
  }
